...
namespace OEngine\LaravelPackage\Plugin;
 ...
 * {
 *     "require": {
 *         "oengine/laravel-package": "dev-master"
 *     },
 *     "extra": {
 *         "merge-plugin": {
 *             "include": [
...
class MergePlugin implements PluginInterface, EventSubscriberInterface
{
     ...
     */
    public const PACKAGE_NAME = 'oengine/laravel-package';
     ...
     * @throws \OEngine\LaravelPackage\Exceptions\MissingFileException when required and a pattern returns no
     *      results
     */
    protected function mergeFiles(array $patterns, $required = false)
    {
        ...
        $files = array_map(
            static function ($files, $pattern) use ($required) {
                if ($required && !$files) {
                    throw new \OEngine\LaravelPackage\Exceptions\MissingFileException(
